...
import { fetchFiles, getHardDrivePath } from '../services/FilesService';
...
const HardDrivePage: React.FC = () => {
  ...
  const fetchHardDrivePath = async () => {
    try {
      const harddrivePath = await getHardDrivePath(); // Fetch desktop path from API
      console.log('Fetched desktop path:', harddrivePath); // Add logging
      setCurrentPath(harddrivePath);
    } catch (err) {
