...
import { fetchFiles, getDesktopPath } from '../services/FilesService';
...
const DesktopPage: React.FC = () => {
  ...
  const fetchDesktopPath = async () => {
    try {
      const desktopPath = await getDesktopPath(); // Fetch desktop path from API
      console.log('Fetched desktop path:', desktopPath); // Add logging
  ...
  if (!currentPath) {
    return <div>Loading desktop...</div>; // Show loading message while fetching path
  }
